...
const STORAGE_KEY = "secretariatmedicalquest_state_v1";
...
const ACCESS_CODES = ["SECMED-2026-AT71"]; // code(s) fourni(s) au centre de formation après achat de la licence
...
const SYNC_KEY = "secretariatmedicalquest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("secretariatmedicalquest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("secretariatmedicalquest_device_id", id);
  }
  return id;
}
